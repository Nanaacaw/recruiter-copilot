...
def _build_screening_dict(s: Screening) -> dict:
    return {
...
def export_screening_pdf(screening_id: str, db: Session = Depends(get_db)):
    ...
    export_service.generate_screening_pdf(
        screening=_build_screening_dict(screening),
        candidate={"name": screening.candidate.name, "email": screening.candidate.email},
        jd={"title": screening.job_description.title, "department": screening.job_description.department},
...
def export_excel(jd_id: str, db: Session = Depends(get_db)):
    ...
    if not screenings:
        raise HTTPException(status_code=404, detail="No screenings found for this job description")

    tmp_dir = tempfile.mkdtemp()
    output_path = os.path.join(tmp_dir, f"batch_{jd_id}.xlsx")

    export_service.generate_excel_report([_build_screening_dict(s) for s in screenings], output_path)
